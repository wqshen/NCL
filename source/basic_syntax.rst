...
.. note:: 需要认真提到的是，你必须使用双引号来定义字符串，单引号是不被允许的。并且NCL中并没有转义字符，这多少会带来一些麻烦，比如你的字符串中本身包含双引号。官方提出的办法是你可以使用函数str_get_dq或者tochar(34)来生成一个引号，并用 + 来连接，比如 dq = str_get_dq() a = "My name is "+dq+"wqshen"+dq 。
